WITH date_dim AS (select *
                  from {{ source('external_source', 'date_dim') }}),
     catalog_sales AS (select *
                       from {{ source('external_source', 'catalog_sales') }}),
     web_sales AS (select *
                   from {{ source('external_source', 'web_sales') }}),
     wscs as
     ...
     wswscs AS
         (SELECT date_dim.d_week_seq d_week_seq,
          ...
          GROUP BY d_week_seq)
SELECT d_week_seq1 d_week_seq1
